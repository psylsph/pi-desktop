...
A cross-platform desktop application for the [pi coding agent](https://www.npmjs.com/package/@earendil-works/pi-coding-agent), built with Electron and TypeScript.
...
- **Thinking level controls** — Adjust agent thinking depth (off, low, medium, high)
...
- Node.js 18+
- An API key for at least one supported provider (e.g., `ANTHROPIC_API_KEY`)
...
Compiles TypeScript, copies renderer assets to `dist/`, and launches the Electron app.
...
| `npm run pack`     | Build unpacked directory (no installer) |
...
The app automatically uses these on launch.
